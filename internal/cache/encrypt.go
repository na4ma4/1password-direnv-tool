...
func (e *Encryption) Iterate(ctx context.Context, fn IterateFunc) error {
	return e.cc.Iterate(ctx, func(key string, age time.Time, value string) error {
		decoded, err := e.codec.Decode(value)
		if err != nil {
			return err
		}

		return fn(key, age, decoded)
	})
}
...
func (w *encryptionWriter) Close() error {
	...
	return w.cc.Set(w.ctx, w.key, w.buf.String())
}
